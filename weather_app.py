...
class Weather():
        ...
        def weather_report(self):
                ...
                self.url = r"http://api.openweathermap.org/data/2.5/weather?q="
                ...
                if self.api_key == '':
                        self.root.withdraw()
                        # the input dialog
                        apikeyByUsr = simpledialog.askstring(title="Error",
                                                        prompt="Please Enter Your API Key\nGo to the source code and change the API Key\nvisit: openweathermap.org for more api key")
                        if len(apikeyByUsr) == 32:
                                with open("apikey.txt","w") as f:
                                        f.write(apikeyByUsr)
                                self.api_key = apikeyByUsr
                        else:
